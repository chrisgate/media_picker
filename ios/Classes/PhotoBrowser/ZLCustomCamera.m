...
CGFloat radians(CGFloat degress) { return degress * M_PI / 180; };
...
- (void)viewDidLayoutSubviews
{
    ...
    //剪裁框布局
    CGFloat width = GetViewWidth(self.view);
    CGRect cropFrame = CGRectZero;
    switch (self.cropType) {
        case ZLCropTypeIDFront: {
            CGFloat cropViewWidth =  width - 16 * 2;
            CGFloat cropViewHeight = cropViewWidth / 690.0 * 451.0;
            cropFrame = CGRectMake(16, 120, cropViewWidth, cropViewHeight);
        }
            break;
            
        case ZLCropTypeIDBack: {
            CGFloat cropViewWidth =  width - 16 * 2;
            CGFloat cropViewHeight = cropViewWidth / 690.0 * 451.0;
            cropFrame = CGRectMake(16, 120, cropViewWidth, cropViewHeight);
        }
        ...
        case ZLCropTypeIDHand: {
            CGFloat cropViewWidth =  width - 16 * 2;
            CGFloat cropViewHeight = cropViewWidth / 682.0 * 812.0;
            cropFrame = CGRectMake(16, 80, cropViewWidth, cropViewHeight);
        }
            break;
            
        case ZLCropTypeDebitcard: {
            CGFloat cropViewWidth =  width - 16 * 2;
            CGFloat cropViewHeight = cropViewWidth / 682.0 * 443.0;
            cropFrame = CGRectMake(16, 120, cropViewWidth, cropViewHeight);
        }
            break;
        default:
    ...
    if (self.cropType != ZLCropTypeNone) {
        self.cropImageView.frame = cropFrame;
        [self.view bringSubviewToFront:self.cropImageView];
    }
}
...
- (void)setupUI
{
    ...
    switch (self.cropType) {
        case ZLCropTypeIDFront: {
            self.cropImageView.image = GetImageWithName(@"ID_fronttips");
            [self.cropImageView setHidden:NO];
        }
            break;
        case ZLCropTypeIDBack: {
            self.cropImageView.image = GetImageWithName(@"ID_reversetips");
            [self.cropImageView setHidden:NO];
        }
            break;
        case ZLCropTypeIDHand: {
            self.cropImageView.image = GetImageWithName(@"ID_handtips");
            [self.cropImageView setHidden:NO];
        }
            break;
        case ZLCropTypeDebitcard: {
            self.cropImageView.image = GetImageWithName(@"bank_cardtips");
            [self.cropImageView setHidden:NO];
        }
            break;
...
- (void)onOkClick
{
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        if (self.doneBlock) {
            ...
            CGRect originalRect = self.takedImageView.frame;
            CGRect clipRect = self.cropImageView.frame;
            UIImage *image = [self clipImage:self.takedImage originalRect:originalRect clipRect:clipRect];
            self.doneBlock(image, self.videoUrl);
        }
    }];
}

// 根据剪裁框剪裁图片
- (UIImage *)clipImage:(UIImage *)originalImage originalRect:(CGRect)originalRect clipRect: (CGRect) clipRect {
    
    originalImage = [UIImageFixTool fixOrientation:originalImage];
    clipRect = [self.view convertRect:clipRect toView:self.takedImageView];
    CGFloat deviceScale = [UIScreen mainScreen].scale;
    
    CGFloat x = clipRect.origin.x * (originalImage.size.width / originalRect.size.width);
    CGFloat y = clipRect.origin.y * (originalImage.size.height / originalRect.size.height);
    CGFloat w = clipRect.size.width * (originalImage.size.width / originalRect.size.width);
    CGFloat h = clipRect.size.height * (originalImage.size.height / originalRect.size.height);
    CGRect cropRect = CGRectMake(x, y, w, h);
    CGImageRef imgRef = CGImageCreateWithImageInRect(originalImage.CGImage, cropRect);
    UIGraphicsBeginImageContextWithOptions(clipRect.size, 0, deviceScale);
    CGContextRef context = UIGraphicsGetCurrentContext();
    CGContextTranslateCTM(context, 0, clipRect.size.height);
    CGContextScaleCTM(context, 1, -1);
    CGContextDrawImage(context, CGRectMake(0, 0, clipRect.size.width, clipRect.size.height), imgRef);
